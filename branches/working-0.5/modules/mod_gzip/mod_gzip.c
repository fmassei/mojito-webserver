    ...
    for (el=cfg_mod->settings->head; el!=NULL; el=el->next) {
        ...
        if (!strcmp(set->key, "chunk_length")) {
            ...
            if ((errno==ERANGE
                ...
                DBG_PRINT(("[mod_gzip] Invalid setting %s = %s\n",
                            set->key, set->val));
            else
                s_chunk_length = tmpl;
            DBG_PRINT(("[mod_gzip] chunk_length set to %d\n", s_chunk_length));
        } else if (!strcmp(set->key, "use_on_mime")) {
            char *copyval;
            if (set->val==NULL) continue;
            if ((copyval = xstrdup(set->val))==NULL) {
                DBG_PRINT(("[mod_gzip] could not set mime %s\n", set->val));
                continue;
            }
...
static t_module_ret_e _on_send(t_response_s *res)
{
    ...
    size_t written, have;
    state = state_get(res);
    DBG_PRINT(("[mod_gzip] gzip on send.\n"));
    if (state==NULL) {
        if ((state = state_create(res))==NULL) {
            DBG_PRINT(("[mod_gzip] Could not create gzip state\n"));
            return MOD_ERR;
        }
